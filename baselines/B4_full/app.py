"""B4 full baseline: PoP + ctx tolerance + exchange gating + adaptive throttle."""
...
class BudgetManager:
    ...
    def precharge(self, token: str, precharge_tokens: int, risk: float, restricted: bool) -> tuple[bool, int, int]:
        state = self._get_state(token)
        before = state.tpm_remaining
        shrink = math.exp(-3.2 * risk) * (0.55 if restricted else 1.0)
...
def _decode_token(token: str) -> tuple[bool, str, str, str, bool]:
    parts = token.split(".")
    ...
    try:
        ...
        _, exp_s, jkt, ctx_hash, restricted_s = decoded.split("|", maxsplit=4)
        exp = int(exp_s)
    ...
    if int(time.time()) > exp:
        return (False, "token_invalid", "", "", False)
    return (True, "ok", jkt, ctx_hash, restricted)
...
def _exchange_risk(profile: Profile | None, ctx: Ctx) -> float:
    if profile is None:
        return 0.0
    risk = 0.0
    if profile.country != ctx.country:
        risk += 0.55
    if profile.asn != ctx.asn:
        risk += 0.40
    if profile.device_fp != ctx.device_fp:
        risk += 0.30
    if profile.ua_major != _ua_major(ctx.ua):
        risk += 0.10
...
def _load_thresholds() -> dict[str, float]:
    defaults = {
        "tau_allow": 0.20,
        "tau_deny": 0.75,
        "tau_allow_exchange": 0.35,
        "tau_deny_exchange": 0.70,
    }
...
def create_app() -> FastAPI:
    ...
    def chat_completions(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not access_token:
            return log_and_return(401, "deny", "token_missing", 0, 0, -1, -1, 1.0, {"error": "token_missing"})

        valid, reason, expected_jkt, expected_ctx_hash, restricted = _decode_token(access_token)
        if not valid:
            return log_and_return(401, "deny", reason, 0, 0, -1, -1, 0.95, {"error": reason})
        ...
        risk = min(1.0, 0.06 * pressure + (0.12 if restricted else 0.0))

        dpop_ok, dpop_reason = _verify_dpop(dpop, access_token, expected_jkt, replay_cache)
        if not dpop_ok:
            return log_and_return(401, "deny", dpop_reason, 0, 0, -1, -1, max(risk, 0.85), {"error": dpop_reason})
        ...
        if exchange_ctx is None or _ctx_hash(exchange_ctx) != expected_ctx_hash:
            return log_and_return(401, "deny", "ctx_mismatch", 0, 0, -1, -1, max(risk, 0.9), {"error": "ctx_mismatch"})

        tolerated, base_risk, mismatch_reason = _ctx_tolerance(exchange_ctx, req_ctx)
        risk = min(1.0, base_risk + 0.22 * pressure + (0.10 if restricted else 0.0))
        if not tolerated:
            return log_and_return(401, "deny", mismatch_reason or "ctx_mismatch", 0, 0, -1, -1, risk, {"error": "ctx_mismatch"})

        precharge_tokens = precharge_tokens_requested
        if risk >= thresholds["tau_deny"]:
        ...
        if risk >= thresholds["tau_allow"] or restricted:
            cap = 0.45 if not restricted else 0.30
            capped = max(4, int(precharge_tokens * cap * math.exp(-1.5 * risk)))
            max_tokens_cap_applied = capped < precharge_tokens
            precharge_tokens = capped

        effective_budget_reduced = restricted or risk >= thresholds["tau_allow"]
        server_delay_applied = pressure > 0.12
        applied_throttle = max_tokens_cap_applied or effective_budget_reduced or server_delay_applied

        allowed, budget_before, budget_after_pre = budgets.precharge(access_token, precharge_tokens, risk, restricted)
        if not allowed:
            return log_and_return(
                429,
                "throttle",
        ...
        success_decision = "throttle" if applied_throttle else "allow"
        return log_and_return(
            200,
            success_decision,
            "ok",
