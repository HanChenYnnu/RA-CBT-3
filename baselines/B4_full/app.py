...
def create_app() -> FastAPI:
    ...
    def chat_completions(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not dpop_ok:
            return log_and_return(401, "deny", dpop_reason, 0, 0, -1, -1, 1.0, {"error": dpop_reason})
        ...
        if exchange_ctx is None or _ctx_hash(exchange_ctx) != expected_ctx_hash:
            return log_and_return(401, "deny", "ctx_mismatch", 0, 0, -1, -1, 1.0, {"error": "ctx_mismatch"})

        tolerated, base_risk, mismatch_reason = _ctx_tolerance(exchange_ctx, req_ctx)
        if not tolerated:
            return log_and_return(401, "deny", mismatch_reason or "ctx_mismatch", 0, 0, -1, -1, 1.0, {"error": "ctx_mismatch"})
        ...
        if risk >= thresholds["tau_allow"] or restricted:
            ...
            precharge_tokens = max(4, int(precharge_tokens * cap * math.exp(-1.5 * risk)))
