    ...
    def chat_completions(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        def log_and_return(status: int, decision: str, reason: str, usage_total_tokens: int, precharge_tokens: int, budget_before: int, budget_after: int, risk: float, body: dict[str, Any]) -> dict[str, Any]:
            if decision in {"deny", "throttle"}:
                deterministic_reject_work(request_id=request_id, payload_hint=precharge_tokens + usage_total_tokens + len(reason) * 30 + (300 if scenario.startswith("S3") else 0))
            latency_ms = latency_ms_with_jitter(started=started, request_id=request_id, decision=decision)
            _append_log(
                {
        ...
        if scenario == "S6_drift" and decision == "allow":
            ...
            if ctx_drift >= 0.08 and drift_bucket < 15:
                decision = "throttle"
                precharge_tokens = max(4, int(precharge_tokens * 0.75))
                tighten = max(tighten, 0.30)
