...
def run_b4_scenario(*, scenario: str, n: int, log_path: Path, seed: int) -> list[EventRow]:
    ...
    with TestClient(app) as client:
        ...
        if scenario in {"S1_key_leak", "S1_key_leak_hard"}:
            attacker_ctx = _ctx("10.0.0.99", "AS100", "US", "browser/100.9", "fp-1" if scenario == "S1_key_leak_hard" else "atk-fp")
            ...
            for idx, req in enumerate(reqs):
                ...
                if token and (scenario == "S1_key_leak_hard" or idx % 3 == 0):
                    headers["DPoP"] = _proof(ATTACKER_PRIVATE, token, f"s1-{idx}", jkt)
                validate_request_semantics(scenario=scenario, auth_present=bool(token), dpop_present="DPoP" in headers, exchange_called=True)
                record_sample(scenario=scenario, baseline="B4", caps=manifest, auth_present=bool(token), dpop_present="DPoP" in headers, exchange_called=True, replay_key="")
            ...
            for idx, req in enumerate(reqs):
                ...
                if scenario == "S2_token_leak_hard" and idx < 5:
                    headers["DPoP"] = _proof(LEGIT_PRIVATE, owner_token, f"s2-hard-captured-{idx}", owner_jkt)
                ...
                validate_request_semantics(scenario=scenario, auth_present=True, dpop_present="DPoP" in headers, exchange_called=False)
                record_sample(scenario=scenario, baseline="B4", caps=manifest, auth_present=True, dpop_present="DPoP" in headers, exchange_called=False, replay_key="")
            ...
            for idx, req in enumerate(reqs):
                if scenario == "S3_replay_hard" and idx % 6 == 0:
                    replay = _proof(LEGIT_PRIVATE, owner_token, f"s3-replay-hard-{idx//6}", owner_jkt)
                replay_key = json.loads(replay).get("jti", "")
                validate_request_semantics(scenario=scenario, auth_present=True, dpop_present=True, exchange_called=False)
                record_sample(scenario=scenario, baseline="B4", caps=manifest, auth_present=True, dpop_present=True, exchange_called=False, replay_key=replay_key)
                client.post("/v1/chat/completions", json=req, headers={"Authorization": f"Bearer {owner_token}", "DPoP": replay, "X-Forwarded-For": "10.0.0.11", "X-CTX": owner_ctx})
            ...
            for idx, req in enumerate(reqs):
                ...
                validate_request_semantics(scenario=scenario, auth_present=True, dpop_present=True, exchange_called=True)
                record_sample(scenario=scenario, baseline="B4", caps=manifest, auth_present=True, dpop_present=True, exchange_called=True, replay_key="")
                client.post("/v1/chat/completions", json=req, headers=headers)
