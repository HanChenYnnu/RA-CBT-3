    ...
    with TestClient(app) as client:
        ...
        if scenario in {"S1_key_leak", "S1_key_leak_hard"}:
            ...
            for idx, req in enumerate(reqs):
                ...
                if scenario == "S1_key_leak_hard" or idx % 3 == 0:
            ...
            for idx, req in enumerate(reqs):
                ...
                if scenario == "S2_token_leak_hard":
                    if idx % 3 == 0:
                        headers["DPoP"] = _proof(LEGIT_PRIVATE, token, f"s2-hard-{idx}", str(ex["cnf"]["jkt"]))
                    else:
                        headers["DPoP"] = _proof(ATTACKER_PRIVATE, token, f"s2-hard-{idx}", "wrong-jkt")
                elif scenario == "S2b_token_leak_wrong_key_dpop" or (
            ...
            for idx, req in enumerate(reqs):
                if scenario == "S3_replay_hard" and idx % 3 == 0:
                    replay = _proof(LEGIT_PRIVATE, token, f"s3-replay-hard-{idx//3}", jkt)
                client.post(
                    "/v1/chat/completions",
                    json=req,
                    headers={
