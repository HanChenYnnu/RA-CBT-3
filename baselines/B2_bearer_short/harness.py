...
def run_b2_scenario(*, scenario: str, n: int, log_path: Path, seed: int) -> list[EventRow]:
    ...
    with TestClient(app) as client:
        ...
        for req in requests:
            ...
            validate_request_semantics(scenario=scenario, auth_present=True, dpop_present=False, dpop_valid=False, exchange_called=exchange_called)
            record_sample(scenario=scenario, baseline="B2", caps=manifest, auth_present=True, dpop_present=False, dpop_valid=False, exchange_called=exchange_called, replay_key="", asn="AS100", country="US", ua_family="browser", max_tokens=int(req.get("max_tokens", 0)))
