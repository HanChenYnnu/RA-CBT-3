...
def run_b3_scenario(*, scenario: str, n: int, log_path: Path, seed: int) -> list[EventRow]:
    ...
    with TestClient(app) as client:
        ...
        if scenario in {"S1_key_leak", "S1_key_leak_hard"}:
            ...
            token, _ = _exchange(client, LEGIT_JWK)
            for idx, req in enumerate(requests):
                headers = {"Authorization": f"Bearer {token}"}
                if scenario in {"S2_token_leak_hard", "S2b_token_leak_wrong_key_dpop"} or (
            ...
            for idx, req in enumerate(requests):
                if scenario == "S3_replay_hard" and idx % 5 == 0:
                    replay = _proof(LEGIT_PRIVATE, token, f"s3-replay-hard-{idx//5}", jkt)
                client.post(
                    "/v1/chat/completions",
                    json=req,
