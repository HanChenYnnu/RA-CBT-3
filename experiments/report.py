...
def write_report(
    ...
    *,
    ...
    header = (
        ...
        "throttle_rate_mean,throttle_rate_std,p50_ms_mean,p50_ms_std,p95_ms_mean,p95_ms_std,risk_p50_mean,risk_p50_std,risk_p90_mean,risk_p90_std"
    )
    lines = [header]
    for row in rows:
        lines.append(
    ...
    if risk_summary:
        ...
        for group, stats in risk_summary.items():
            markdown.append(
                f"| {group} | {int(stats['n'])} | {stats['p50']:.4f} | {stats['p90']:.4f} |"
            )
