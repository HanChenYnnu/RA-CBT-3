"""Metrics computation with multi-seed aggregation, ROC, and calibration quality."""
...
def compute_b4_calibration_metrics(events: list[EventRow]) -> tuple[float, float, list[ReliabilityBin]]:
    pts = [e for e in events if e.baseline == "B4" and e.risk >= 0.0]
    if not pts:
        return (0.0, 0.0, [])
    labels = [1.0 if e.label == "attack" else 0.0 for e in pts]
    preds = [e.risk for e in pts]
    ...
    for i in range(10):
        lo = i / 10.0
        hi = (i + 1) / 10.0
        bucket = [(p, y) for p, y in zip(preds, labels) if (lo <= p < hi) or (i == 9 and p == 1.0)]
        if not bucket:
            continue
        mean_pred = sum(p for p, _ in bucket) / len(bucket)
        emp = sum(y for _, y in bucket) / len(bucket)
        count = len(bucket)
        ece += abs(mean_pred - emp) * (count / len(preds))
