...
def generate_plots(rows: list[MetricRow], *, b4_eval: B4RiskEvaluation) -> list[Path]:
    ...
    out.append(_line_svg("B4 Allowed-only Precision-Recall", {"Allowed PR": [(p.x, p.y) for p in b4_eval.allowed_pr_points]}, "Recall", "Precision", PLOTS_DIR / "b4_allowed_pr.svg"))
    ...
    out.append(_line_svg("Burst ASR vs Load Level", {
    ...
    }, "Load level", "ASR", PLOTS_DIR / "burst_asr_curve.svg"))
